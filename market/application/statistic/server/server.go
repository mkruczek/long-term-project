...
func New(services appServices) *Server {
	return &Server{
		Server:   baseServer.New(config.GetStatistic().Http.URLPath, config.GetStatistic().Http.Port),
		services: services,
	}
}
...
func (svr *Server) Init(ctx context.Context) {
	...
	db, err := mongo.New(ctx, config.GetStatistic().DataBase.DBName, config.GetStatistic().DataBase.Host, config.GetStatistic().DataBase.Port, config.GetStatistic().DataBase.User, config.GetStatistic().DataBase.Password)
	if err != nil {
