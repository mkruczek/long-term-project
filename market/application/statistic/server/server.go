...
func New(services appServices) *Server {
	return &Server{
		Server:   baseServer.New(config.GetStatistic().Http.URLPath, config.GetTrade().Http.Port),
		services: services,
	}
}
...
func (svr *Server) Init(ctx context.Context) {
	...
	db, err := mongo.New(ctx, config.GetStatistic().DataBase.DBName, config.GetTrade().DataBase.Host, config.GetTrade().DataBase.Port, config.GetTrade().DataBase.User, config.GetTrade().DataBase.Password)
	if err != nil {
